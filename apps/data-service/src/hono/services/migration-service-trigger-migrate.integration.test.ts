...
describe("triggerMigrate (integration)", () => {
	...
	it("defaults dryRun to false when not provided and forwards body without dryRun field omitted? (sends dryRun:false)", async () => {
		...
		expect(body.runnerConfig).toMatchObject({ b2KeyId: expect.any(String) });
	});
	...
	it("posts to runner /jobs/migrate with dryRun=true, persists migration_jobs row with type=migrate", async () => {
		const deploymentId = await setupReadyDeployment();
		const user = await createTestUser();
		const runnerJobId = randomUUID();
		mockRunnerJobAccept(runnerJobId);
		...
		expect(result.data.type).toBe("migrate");
		...
		expect(urlStr).toBe("https://runner.example.com/jobs/migrate");
	...
	it("returns JOB_ALREADY_RUNNING when an active backup exists (cross-type), does not call runner", async () => {
		const deploymentId = await setupReadyDeployment();
		const user = await createTestUser();
